...
delta_t = 10; % Size of the time-interval in which minimization of the maximum number of vehicles at the depot is to be done.
load('solution','T','nk','wFinishTimes','td','journeyTimes');
% load('solution_uncogestion_sample');
timeLeverage = 200; % Additional time leverage on top of the maximum journey time to minimize the congestion at the depot
...
% Unknowns - [x^1_11 x^1_12 x^1_21 x^1_22 x^2_11 y^1_1,1... y^1_1,24 y^1_2,1... y^1_2,24 y^2_1,1... y^2_1,24 c^1_1, c^1_2, c^2_1, n_bar, st^1_1, st^1_2, st^2_1, st_bar]
f = [zeros(nx,1);zeros(nr*nI,1);zeros(nr,1);10000;zeros(nr,1);1]; 
...
% Inequality constraint due to max operation (w.r.t st_bar) in objective function
Aineq3 = zeros(nv,nu);
for ct=1:nv
    ct_col = nx+(nr*nI)+nr+1+sum(nk(1:ct-1));
    Aineq3(ct,ct_col+1:ct_col+nk(ct)) = ones(1,nk(ct));
end
...
lb = [zeros(nx+(nr*nI)+nr,1); 1; zeros(nr,1); 0];
ub = [ones(nx+nr*nI,1); T*ones(nr,1); nv; ub_slackTimes; max(ub_sumSlackTimes)];
